...
def predict(
    ...
    if task_name in ["S2ST", "S2TT", "ASR"]:
        ...
        if new_arr.shape[1] > max_length:
            ...
            gr.Warning(
                "Input audio is too long. Only the first"
                f" {MAX_INPUT_AUDIO_LENGTH} seconds is used."
            )
        torchaudio.save(
            input_data, new_arr, sample_rate=int(AUDIO_SAMPLE_RATE)
        )
    else:
    ...
    with gr.Row(visible=False) as t2st_example_row:
        t2st_examples = gr.Examples(
            examples=[
                ...
                [
                    (
                        "Meta AI's Seamless M4T model is democratising spoken"
